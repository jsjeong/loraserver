...
func (t *UplinkIntegrationTestSuite) initConfig() {
	...
	p := common.NewRedisPool(testConfig.RedisURL)
...
func (t *UplinkIntegrationTestSuite) GetUplinkFrameForFRMPayload(rxInfo gw.UplinkRXInfo, txInfo gw.UplinkTXInfo, mType lorawan.MType, fPort uint8, data []byte, fOpts ...lorawan.Payload) gw.RXPacket {
	...
	var phyNew lorawan.PHYPayload
	b, err := phy.MarshalBinary()
	t.Require().Nil(err)
	t.Require().Nil(phyNew.UnmarshalBinary(b))

	dr, err := config.C.NetworkServer.Band.Band.GetDataRate(txDR)
	t.Require().Nil(err)

	return gw.RXPacket{
		RXInfo: gw.RXInfo{
			MAC:       t.Gateway.MAC,
			Frequency: int(txInfo.Frequency),
			Channel:   txChan,
			DataRate:  dr,
